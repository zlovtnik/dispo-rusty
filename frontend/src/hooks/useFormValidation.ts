...
function formReducer<T extends Record<string, unknown>>(
  ...
  switch (action.type) {
    ...
    case 'SET_VALUES': {
      ...
      for (const [key, value] of Object.entries(action.values) as Array<[keyof T, T[keyof T]]>) {
...
export function useFormValidation<T extends Record<string, unknown>>(
  ...
  const validateFieldValue = useCallback(
    <K extends keyof T>(field: K, value: T[K] | undefined) => {
  ...
  const formState = useMemo(() => {
    const allErrors: Record<keyof T, FormValidationError[]> = {} as Record<keyof T, FormValidationError[]>;
    let isValid = true;
    let isDirty = false;

    for (const key of Object.keys(fieldStates) as Array<keyof T>) {
  ...
  const actions = useMemo(() => ({
    ...
    validateForm: () => {
      ...
      const errors: Record<keyof T, FormValidationError[]> = {} as Record<keyof T, FormValidationError[]>;
    ...
    setValues: (newValues: Partial<T>) => {
      ...
      if (mode === 'onChange') {
        queueMicrotask(() => {
          for (const [key, value] of Object.entries(newValues) as Array<[keyof T, T[keyof T]]>) {
